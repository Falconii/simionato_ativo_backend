...
router.get(
    "/api/inventario/:id_empresa/:id_filial/:codigo",
    async function(req, res) {
        try {
            const lsLista = await inventarioSrv.getInventario(
...
router.get(
    "/api/resumo/:id_empresa/:id_filial/:codigo",
    async function(req, res) {
        try {
            const lsLista = await inventarioSrv.getResumo(
                req.params.id_empresa,
                req.params.id_filial,
                req.params.codigo
            );
            if (lsLista == null) {
                res.status(409).json({ message: "Resumo Não Encontrado." });
            } else {
                res.status(200).json(lsLista);
            }
        } catch (err) {
...
router.get("/api/inventarios", async function(req, res) {
    try {
        ...
        if (err.name == "MyExceptionDB") {
            ...
            res
                .status(500)
                .json({ erro: "BAK-END", tabela: "inventario", message: err.message });
        }
    }
});
...
router.delete(
    ...
    async function(req, res) {
        try {
            await inventarioSrv.deleteInventario(
                req.params.id_empresa,
                req.params.id_filial,
                req.params.codigo
            );
...
router.post("/api/inventarios", async function(req, res) {
    ...
    try {
        ...
        if (lsRegistros.length == 0) {
            res
                .status(409)
                .json({ message: "Inventario Nenhum Registro Encontrado!" });
        } else {
