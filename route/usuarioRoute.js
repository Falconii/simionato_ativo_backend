...
router.get("/api/ambiente/:id_empresa/:id_usuario", async function(req, res) {
    ...
    //Buscao Padrao
    try {
        const padrao = await padraoSrv.getPadrao(
            req.params.id_empresa,
        ...
        if (err.name == "MyExceptionDB") {
            res
                .status(500)
                .json({ erro: "BAK-END", tabela: "padrao", message: err.message });
        }
    }
    ...
    if (PADRAO !== null) {
        try {
            const usuario = await usuarioSrv.getUsuario(
    ...
    if (PADRAO !== null) {
        try {
            const local = await localSrv.getLocal(
                PADRAO.id_empresa_padrao,
            ...
            if (err.name == "MyExceptionDB") {
                res
                    .status(500)
                    .json({ erro: "BAK-END", tabela: "local", message: err.message });
            }
        }
    }
...
router.post("/api/usuariosinventario", async function(req, res) {
    ...
    try {
        const params = req.body;
        const lsRegistros = await usuarioSrv.getUsuariosInventario(
            params.id_empresa,
            params.id_local,
...
router.post("/api/usuariosbyambiente", async function(req, res) {
    console.log("/api/usuariosbyambiente");
    /*
        	{
        		"id_empresa":0, 
        		"id":0, 
    ...
    try {
        const params = req.body;
        const lsRegistros = await usuarioSrv.getUsuariosByAmbiente(params);
        if (lsRegistros.length == 0) {
            res.status(409).json({ message: "Usuario Nenhum Registro Encontrado!" });
        } else {
            res.status(200).json(lsRegistros);
