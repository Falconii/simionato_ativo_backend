...
/* ROTA GETONE foto */
router.get("/api/foto/:id_empresa/:id_local/:id_inventario/:id_imobilizado/:id_pasta/:id_file/:file_name", async function(req, res) {
        try {
            const lsLista = await fotoSrv.getFoto(req.params.id_empresa, req.params.id_local, req.params.id_inventario, req.params.id_imobilizado, req.params.id_pasta, req.params.id_file, req.params.file_name);
            if (lsLista == null) {
...
router.post("/api/uploadfoto", uploadFotos.single("file"), async function(req, res) {
    ...
    const file_name = `${req.body.id_empresa.padStart(2,'0')}_${req.body.id_local.padStart(6,'0')}_${req.body.id_inventario.padStart(6,'0')}_${req.body.id_imobilizado.padStart(6,'0')}_${req.file.originalname}`;
    ...
    let foto = await fotoSrv.getFoto(id_empresa, id_local, id_inventario, id_imobilizado, id_pasta, id_file, file_name);
    ...
    try {
        ...
        if (!existeDrive) {
            ...
            if (ft == null) {
                res.status(200).json({ code: "409", message: 'Falha Ao Incluir Foto No BD' });
            } else {
            ...
            if (ft == null) {
                res.status(200).json({ code: "409", message: 'Falha Ao Alterar Foto No BD' });
            } else {
                try {
                    // Verifincando se existe ok
                    const responseGet = await driveService.files.get({
        ...
        if (err.name == 'MyExceptionDB') {
            ...
            res.status(500).json({ erro: 'BAK-END', tabela: 'fotos', message: err.message });
        }
    }
