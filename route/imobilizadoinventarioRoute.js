...
const lancamentoSrv = require("../service/lancamentoService.js");
const imobilizadoSrv = require("../service/imobilizadoService.js");
  ...
  try {
    ...
    if (emp == null || local == null || inventario == null) {
      ...
      if (lsRegistros == null || lsRegistros.length == 0) {
        ...
        await generateExcel.generateExcelv2(
          lsRegistros,
          inventario,
          complemento,
          fileName,
...
router.post("/api/alter_obs", async function (req, res) {
  ...
  try {
    ...
    const lanca = await lancamentoSrv.getLancamento(
      params.id_empresa,
      params.id_filial,
      params.id_inventario,
      params.id_imobilizado,
    );

    if (emp == null || local == null || inventario == null || lanca == null) {
      res.status(409).json({
        message: "Empresa, Local, Inventário ou Lançamento Não Encontrados!",
      });
    } else {
      const ativo = await imobilizadoSrv.getImobilizado(
        params.id_empresa,
        params.id_filial,
        params.id_imobilizado,
      );
      ...
      if (ativo.origem == "M") {
        ativo.descricao = params.obs;
        ativo.user_update = params.id_usuario;
        await imobilizadoSrv.updateImobilizado(ativo);
      }
