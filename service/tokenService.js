...
if (process.env.FRASE_ACCESS) {
    ...
    console.log("Frase Access Do Parametro: FRASE", ACCESS_SECRET);
} else {
    ACCESS_SECRET = fs.readFileSync("./frase_access.txt", "utf8");
    console.log("Frase Access do arquivo frase.txt", ACCESS_SECRET);
}

if (process.env.FRASE_REFRESH) {
    REFRESH_SECRET = process.env.FRASE_REFRESH;
    console.log("Frase Refresh Do Parametro: FRASE", REFRESH_SECRET);
} else {
    REFRESH_SECRET = fs.readFileSync("./frase_refresh.txt", "utf8");
    console.log("Frase Refresh do arquivo frase.txt", REFRESH_SECRET);
}
...
exports.generateAccessToken = function(user) {
    return jwt.sign({ id_empresa: user.id_empresa, id_usuario: user.id },
        ACCESS_SECRET, { expiresIn: `${validade}d` }
    );
};

exports.generateRefreshToken = function(use) {
    return jwt.sign({ id_empresa: user.id_empresa, id_usuario: user.id },
        REFRESH_SECRET, { expiresIn: `${validade}d` }
    );
};
