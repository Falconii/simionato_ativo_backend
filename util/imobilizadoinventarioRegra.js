...
exports.imobilizadoinventario_Inclusao = async function (
  imobilizadoinventario,
) {
  try {
    console.log("imobilizadoinventario_Inclusao", imobilizadoinventario);
    const obj = await imobilizadoinventarioSrv.getImobilizadoinventario(
...
exports.imobilizadoinventario_Alteracao = async function (
  imobilizadoinventario,
) {
  try {
    const obj = await imobilizadoinventarioSrv.getImobilizadoinventario(
...
exports.imobilizadoinventario_Exclusao = async function (
  id_empresa,
  id_filial,
  id_inventario,
  id_imobilizado,
  ...
  try {
    ...
    if (obj.id_lanca > 0 || obj.fotos > 0) {
      throw new erroDB.UserException("Regra de negócio", [
        {
          tabela: "IMOBILIZADOINVENTARIO",
          message: `"EXCLUSÃO" Registro Não Posso Excluir Pois Existe Lanç. De Inventário Ou Fotos Lançados`,
        },
      ]);
    }
  } catch (err) {
